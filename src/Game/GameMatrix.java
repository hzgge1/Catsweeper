...
public class GameMatrix {
	...
	private void creatCatInMatrix(int numberCat) {
		while (numberCat > 0) {
			int i = (int) (Math.random() * gameMatrix.length);
			int j = (int) (Math.random() * gameMatrix[0].length);
			if (gameMatrix[i][j] != -1) {
	...
	private int getNumberOfCatsInTheNineSquareGrid(int i, int j) {
		...
		for (int k = Math.max(0, i - 1); k <= Math.min(i + 1, gameMatrix.length - 1); k++) {
			for (int k2 = Math.max(0, j - 1); k2 <= Math.min(j + 1, gameMatrix[k].length - 1); k2++) {
				if (gameMatrix[k][k2] == -1)
					count++;
			}
		}
		return count;
